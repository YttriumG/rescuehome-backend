...
import com.rescue.modules.operate.service.LocationService;
...
public class LocationController {
    ...
    public R info(@PathVariable("locationId") String locationId){
        LocationEntity rh_Location = locationService.selectById(locationId);

        return R.ok().put("rh_Location", rh_Location);
    }
    ...
    public R save(@RequestBody LocationEntity rh_Location){
        int lat = rh_Location.getLat().intValue();
        int lng = rh_Location.getLng().intValue();
        rh_Location.setLocationId(rh_Location.getCitycode()+rh_Location.getAdcode()+lat+""+lng+DateUtils.getHHmmssTime());
        rh_Location.setStatus(0);
        rh_Location.setGmtCreate(new Date());
        locationService.insertAllColumn(rh_Location);
        return R.ok();
    }
